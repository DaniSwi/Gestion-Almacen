...
void agregarProductoInventarioUser(
    Map *inventario) { // la funcion recibe el mapa del inventario
...
void cargarProductos(Map *precioProductos) { // la funcion recibe el mapa con el
                                             // precio de los productos
  FILE *archivo =
      fopen("data/productos.csv", "r"); // se abre el archivo que vamos a ocupar
  if (archivo ==
      NULL) { // si el archivo es igual a NULL, es porque no se abrio el archivo
    perror("No se pudo abrir el archivo\n");
    return; // retornamos
  }
  char **campos; // creamos la variable campos que sera cada columna del archivo
  campos = leer_linea_csv(
      archivo, ','); // se lee el archivo con la funcion y separado con una coma
  while ((campos = leer_linea_csv(archivo, ',')) !=
         NULL) { // mientras cada columna sea distinta de NULL
    Producto *producto = crearProducto(); // se crea un nuevo producto
    producto->codigo =
        atoi(campos[0]); // se le asigna el codigo que aparezca en el archivo
    strcpy(producto->nombre,
           campos[1]); // se copia el nombre que hay en el archivo
    formatearCadena(producto->nombre);       // se transforma a mayuscula
    producto->precioVenta = atoi(campos[2]); // copiamos el precio de venta
    strcpy(producto->tipoProducto,
           campos[3]);                       // se copia el el tipo de producto
    formatearCadena(producto->tipoProducto); // se transforma a mayuscula
    map_insert(precioProductos, strdup(producto->nombre),
               producto); // insertamos los datos en el mapa
  }
}
...
void cargarPedidosCSV(List *pedidos, Map *inventario,
  ...
  char **campos;
  campos = leer_linea_csv(
      archivo, ','); // leemos el archivo con la funcion y separado con una coma
  list_clean(pedidos); // limpiamos la lista de pedidos
  while ((campos = leer_linea_csv(archivo, ',')) !=
         NULL) { // mientras cada columna sea distinta de NULL
    Pedido *pedido = crearPedido(); // creamos un nuevo pedido
    pedido->cantidad =
        atoi(campos[0]); // se le asigna la cantidad que aparezca en el archivo
    strcpy(pedido->producto.nombre,
           campos[1]); // se copia el nombre que hay en el archivo
    formatearCadena(pedido->producto.nombre); // se transforma a mayuscula
    MapPair *pair = map_search(
        productosPrecio,
        pedido->producto.nombre); // se busca el puntero pair en el mapa
    if (pair) {                   // si pair existe
      Producto *producto =
          (Producto *)pair->value; // se accede a la lista de productos
      pedido->producto.precioVenta =
          producto->precioVenta; // se le asigna el precio de venta
      pedido->producto.codigo = producto->codigo; // se le asigna el codigo
      strcpy(pedido->producto.tipoProducto,
             producto->tipoProducto); // se copia el el tipo de producto
      pedido->monto = pedido->producto.precioVenta *
                      pedido->cantidad; // se calcula el monto
      list_pushBack(pedidos,
                    pedido); // se agrega el pedido a la lista de pedidos
    }
  }
  fclose(archivo); // se cierra el archivo
  actualizarPedidos(inventario, pedidos);
  Pedido *aux = (Pedido *)list_first(
      pedidos); // se crea un puntero auxiliar para recorrer la lista de pedidos
...
void agregarProductoAuto(
  ...
  MapPair *pair = map_search(
      inventario, &producto->codigo); // Busca el producto en el inventario
  if (pair) { // Si el producto ya existe en el inventario
    List *listaProductos =
        (List *)
            pair->value; // Obtiene la lista de productos asociada al producto
    list_pushBack(listaProductos,
                  producto); // Agrega el producto a la lista de productos
  } else {                   // Si el producto no existe en el inventario
    List *listaProductos = list_create(); // Crea una nueva lista de productos
    list_pushBack(listaProductos,
                  producto); // Agrega el producto a la lista de productos
    map_insert(inventario, &producto->codigo,
               listaProductos); // Inserta el producto en el inventario
  }
}
...
void cargarCargaDesdeCSV(
  ...
  while ((campos = leer_linea_csv(archivo, ',')) !=
    ...
    Producto *producto = crearProducto(); // creamos un nuevo producto
    strcpy(producto->nombre,
           campos[0]); // se copia el nombre que hay en el archivo
    formatearCadena(producto->nombre);       // se transforma a mayuscula
    ...
    MapPair *pair =
        map_search(precioProductos,
                   producto->nombre); // se busca el puntero pair en el mapa
    if (pair) {                       // si pair existe
      Producto *new =
          (Producto *)pair->value;    // se accede a la lista de productos
      producto->codigo = new->codigo; // se le asigna el codigo
      agregarProductoAuto(inventario,
                          producto); // se agrega el producto al inventario
...
void mostrarProductos(Map *productos) { // Muestra los productos
  ...
  printf("================================================\n\n");
  MapPair *pair =
      map_first(productos); // Obtiene el primer par de clave-valor del mapa
  while (pair) {            // Mientras haya pares en el mapa
    Producto *producto =
        (Producto *)pair->value; // Obtiene el valor asociado al par
    printf("================================================\n\n");
    ...
    pair =
        map_next(productos); // Obtiene el siguiente par de clave-valor del mapa
  }
  printf("================================================\n");
}
...
void cuadrarInventario(Map *inventario,
  ...
  printf("================================================\n\n");
  MapPair *pair =
      map_first(inventario); // Obtiene el primer par de clave-valor del mapa
  ...
  while (pair) { // Mientras haya pares en el mapa
    List *listaProductos =
        (List *)
            pair->value; // Obtiene la lista de productos asociada al producto
    Producto *producto = (Producto *)list_first(
        listaProductos);    // Obtiene el primer producto de la lista
    if (producto == NULL) { // Si la lista está vacía
      pair = map_next(
          inventario); // Obtiene el siguiente par de clave-valor del mapa
      continue;
    }
    MapPair *nextPair = map_next(
        inventario); // Obtiene el siguiente par de clave-valor del mapa
    MapPair *par = map_search(productosPrecio,
                              producto->nombre); // Busca el producto en el mapa
    if (par == NULL) {
      ...
      list_clean(
          pair->value); // Limpia la lista de productos asociada al producto
  ...
  for (MapPair *pair = map_first(inventario); pair != NULL;
    ...
    List *listProductos =
        (List *)
            pair->value; // Obtiene la lista de productos asociada al producto
    Producto *producto = (Producto *)list_first(
        listProductos); // Obtiene el primer producto de la lista
    while (producto) {  // Mientras haya productos en la lista
      fprintf(
          archivo, "%zu,%s,%d,%d,%d,%d,%d,%s\n", producto->codigo,
          producto->nombre, producto->precioCosto, producto->precioVenta,
          producto->fechaV.dia, producto->fechaV.mes, producto->fechaV.year,
          producto
              ->tipoProducto); // Escribe los datos del producto en el archivo
      producto = (Producto *)list_next(
          listProductos); // Obtiene el siguiente producto de la lista
    }
  }
...
void buscarProductoPorCodigo(
  ...
  printf("\n=========================================\n");
  MapPair *pair =
      map_search(inventario, &codigo); // Busca el producto en el inventario
...
void actualizarProductosPorTipo(
  ...
  while (pair) {            // Mientras haya pares en el mapa
    ...
    formatearCadena(producto->tipoProducto); // Se transforma a mayuscula
    MapPair *par =
        map_search(productosPorTipo,
                   producto->tipoProducto); // Busca el producto en el mapa
    if (par)                                // Si el producto existe en el mapa
      list_pushBack(par->value,
                    producto); // Agrega el producto a la lista de productos
    else {                     // Si el producto no existe en el mapa
      List *listaProductos = list_create(); // Crea una nueva lista de productos
      list_pushBack(listaProductos,
                    producto); // Agrega el producto a la lista de productos
      map_insert(productosPorTipo, producto->tipoProducto,
                 listaProductos); // Inserta el producto en el mapa
    }
    pair =
        map_next(productos); // Obtiene el siguiente par de clave-valor del mapa
  }
...
void buscarProductosPorTipo(
  ...
  MapPair *pair =
      map_search(productosPorTipo, tipo);       // Busca el tipo en el mapa
  if (pair) {                                   // Si se encuentra el tipo
    ...
    while (producto) {                         // Mientras haya productos
      ...
      producto = (Producto *)list_next(
          listaProductos); // Obtiene el siguiente producto de la lista
    }
...
void headerModificacionPedido(
    void) // Imprime el encabezado de la modificacion de pedido
{
  limpiarPantalla();
  printf("=============================================\n");
...
void mostrarPedido(List *pedidos) { // Muestra el pedido
  ...
  if (size == 0) {
    ...
    while (aux) { // Se imprimen los productos del pedido, junto a su precio de
                  // venta y cantidad
      ...
      aux =
          list_next(pedidos); // Mover puntero del pedido al siguiente producto
    }
    printf("\nCosto total: %d$\n",
               ...
               st codigo) //  Busca en el pedido un producto por su codigo
{
  ...
  while (aux) {
    if (aux->producto.codigo == codigo) {
      return aux; // Se retorna el producto del pedido si se encuentra dentro de
                  // la lista
    }
    ...
    Map *inventario) // Agrega un producto del pedido por su codigo
{
  ...
  printf("\n\n==============================================\n");
  MapPair *pair = map_search(
      inventario, &codigo); // Se busca en el inventario utilizando el codigo
  if (!pair)                // Si no esta el producto en el inventario
    ...
    int sizeInvenProdu =
        list_size(pair->value); // Tamaño/cantidad del producto en el inventario
    Pedido *aux = buscarEnPedido(
        pedidos, codigo); // Se busca el producto en la lista de pedido
    ...
    else // Si el producto está en la lista, la cantidad total es igual a la
         // deseada
      cantTotal = cant;
    if (sizeInvenProdu < cantTotal) { // Si la cantidad deseada es mayor que lo
                                      // que hay disponible
      printf("La cantidad deseada es mayor que la cantidad en inventario\n");
      printf("Se descartará todo el producto del inventario (%d)\n",
             sizeInvenProdu);
      cantTotal = sizeInvenProdu; // Se arregla la cantidad para que sea igual a
                                  // la disponible
    }
    if (aux)
      aux->cantidad = cantTotal; // Se ajusta la cantidad dentro del pedido
    else {
      Pedido *new = crearPedido(); // Se agrega un nuevo producto junto a sus
                                   // datos a la lista de pedido
...
void agregarManual(List *pedidos) // Agrega un producto manualmente al pedido
{
  ...
  list_pushBack(pedidos,
                ped); // Se agregan el producto a la lista y se leen los datos
  printf("Ingrese el nombre del producto: ");
                            ...
                            Map *inventario) // Elimina un producto del pedido
{
  ...
  do {
    ...
    if (numProd == 0)      // Si la opción es 0, termina la función
      ...
      presioneTeclaParaContinuar();
           ...
           numProd < 0); // Itera mientras que la opción no sea valida
  ...
  if (cont == numProd) {
    ...
    if (cant >= aux->cantidad)  // Si la cantidad a eliminar es mayor o igual a
                                // la cantidad de
      list_popCurrent(pedidos); // producto en el pedido, se elimina el producto
                                // por completo
    else if (cant < 0)
      printf("Cantidad no valida\n");
    else {
      aux->cantidad -= cant; // Se actualiza la cantidad del pedido
      aux->monto = aux->producto.precioVenta *
                   aux->cantidad; // Se actualiza el monto del pedido
    }
  }
}
...
void modificarPedido(List *pedidos, Map *inventario) // Modifica el pedido
{
  ...
  do {                             // Se repite hasta que se ingrese 0
    ...
    else if (size > 0 && op == 2) // Si hay productos en el pedido y la opción
                                  // es 2, se llama a la función que correspon
      eliminarProductoPedido(pedidos,
                             inventario); // Eliminar producto del pedido
...
void pagoEfectivo(List *pedidos) // Realiza el pago en efectivo
{
  ...
  while (aux) { // Se imprimen los productos del pedido, junto a su precio de
    ...
    sumaPrecio += (aux->producto.precioVenta *
                   aux->cantidad); // Se suman los montos de los productos
    aux = list_next(pedidos);      // Se mueve el puntero al siguiente producto
  }
  ...
  do {      // Se repite hasta que el pago sea mayor o igual al precio total
    ...
    if (pago < sumaPrecio) { // Si el pago es menor al precio total, se imprime
                             // un mensaje de error
      printf("Faltan %d$\n", sumaPrecio - pago); // Se imprime el monto faltante
      presioneTeclaParaContinuar(); // Se presiona una tecla para continuar
    } else if (pago == sumaPrecio)  // Si el pago es igual al precio total, se
  ...
  } while (pago < sumaPrecio); // Se repite hasta que el pago sea mayor o igual
                               // al precio total
}
...
void eliminarPedido(List *pedidos, Map *inventario) { // Elimina el pedido
  ...
  while (aux) { // Se recorre la lista hasta el último elemento
    MapPair *pair = map_search(
        inventario,
        &aux->producto.codigo); // Se busca el producto en el inventario
    if (pair) {                 // Si se encuentra el producto en el inventario
      if (list_size(pair->value) ==
          aux->cantidad) // Si la cantidad del producto es igual a la cantidad
                         // del pedido
        map_remove(
            inventario,
            &aux->producto.codigo); // Se elimina el producto del inventario
      while (aux->cantidad >
             0) { // Se recorre la lista hasta el último elemento
...
void procesarPago(List *pedidos, Map *inventario) // Procesa el pago
{
  ...
  do {
    ...
    case 1: // Confirmar pedido
      ...
      if (mp >= 1) { // Si los pagos se realizaron exitosamente, se eliminan los
                     // productos del inventario
        eliminarPedido(
            pedidos,
            inventario); // Funcion que elimina los productos del inventario
...
void procesarVenta(List *pedidos, Map *inventario) // Procesa la venta
{
  ...
  do {
    ...
    if (pedidoSize == 0) {               // si el pedido esta vacio
      mostrarMenuPedidoVacio(); // Se llama a la función que muestra el menu de
                                // pedido vacio
      scanf("%d", &op);         // Se lee la opción
      switch (op) {             // Se procesa la opción
      case 1:
        modificarPedido(pedidos,
                        inventario); // SE llama a la funcion modificarPedido
        break;
...
void cargarInventario(Map *inventario) {             // Carga el inventario
  ...
  while ((campos = leer_linea_csv(archivo, ',')) !=
         NULL) {                             // Lee las lineas del archivo
    ...
    if (pair) { // Si el producto ya existe en el inventario
      List *listaProductos =
          (List *)pair->value;                 // Obtiene la lista de productos
      ...
      list_pushBack(listaProductos, producto); // Agrega el producto a la lista
      map_insert(inventario,
                 &producto->codigo, // Inserta el producto en el inventario
...
int main(int argc, char *argv[]) { // Funcion principal
  ...
  Map *precioProductos =
      map_create(is_equal_str); // Crea un mapa de precios de productos
  Map *productosPorTipo =
      map_create(is_equal_str);     // Crea un mapa de productos por tipo
  ...
  actualizarProductosPorTipo(
      productosPorTipo, precioProductos); // Actualiza los productos por tipo
